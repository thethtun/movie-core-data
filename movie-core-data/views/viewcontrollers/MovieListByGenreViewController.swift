...
extension MovieListByGenreViewController : UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let movieDetailsViewController = storyboard?.instantiateViewController(withIdentifier: String(describing : MovieDetailsViewController.self)) as? MovieDetailsViewController {
            let movie = fetchResultController.object(at: indexPath)
...
extension MovieListByGenreViewController: NSFetchedResultsControllerDelegate {
